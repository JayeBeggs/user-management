export async function clickUploadCta(page) {
  ...
  await cta.waitFor({ timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) }).catch(() => {});
  try { await cta.scrollIntoViewIfNeeded(); } catch {}
  try { await cta.click({ timeout: parseInt(process.env.UNIVERSAL_CLICK_TIMEOUT || '3000', 10), force: true }); return true; } catch {}
  ...
  try {
    ...
    await roleBtn.waitFor({ timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) });
...
export async function uploadIdSection(page, sectionHeadingText, filePath) {
  ...
  await heading.waitFor({ timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) });
  ...
  await page.waitForFunction((sel, prev) => document.querySelectorAll(sel).length >= prev, 'input[type="file"]', beforeCount, { timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) }).catch(() => {});
  const afterCount = await inputs.count();
  if (afterCount > 0) {
    const newestIndex = Math.max(0, afterCount - 1);
    targetInput = inputs.nth(newestIndex);
  }
  await targetInput.waitFor({ timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) });
...
export async function clickRecordCircle(page) {
  ...
  await control.waitFor({ timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) }).catch(() => {});
  try { await control.scrollIntoViewIfNeeded(); } catch {}
  for (let i = 0; i < 6; i++) {
    try { await control.click({ timeout: parseInt(process.env.UNIVERSAL_CLICK_TIMEOUT || '3000', 10), force: true }); return true; } catch {}
...
export async function recordLivenessWithRetry(page) {
  const waitForSubmit = async (timeoutMs = 4000) => {
    try {
      ...
      ].join(', ')).first().waitFor({ timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) });
      return true;
    } catch { return false; }
  };
  const doOnce = async () => {
    ...
    await page.waitForTimeout(parseInt(process.env.CAMERA_RECORD_DURATION || '4000', 10));
    let stopped = await clickRecordCircle(page);
    if (!stopped) {
      const elapsed = Date.now() - start;
      const maxDuration = parseInt(process.env.CAMERA_RECORD_MAX_DURATION || '6000', 10);
      if (elapsed < maxDuration) await page.waitForTimeout(maxDuration - elapsed);
      stopped = await clickRecordCircle(page);
    }
    const advanced = await waitForSubmit(parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10));
    return stopped && advanced;
  };
  let ok = await doOnce();
  if (ok) return true;
  try { await page.reload({ waitUntil: 'domcontentloaded' }); } catch {}
  try { await page.locator('button:has-text("I\'m ready"), button:has(h1:has-text("I\'m ready")), [role="button"]:has-text("I\'m ready")').first().click({ timeout: parseInt(process.env.UNIVERSAL_CLICK_TIMEOUT || '3000', 10), force: true }); } catch {}
  ok = await doOnce();
  return ok;
}
