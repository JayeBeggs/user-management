...
import { loginWithPhone, clickStartEarning } from '../utils/app.js';
...
test.describe('Complete Onboarding Flow', () => {
  test('complete personalise my products and start earning flows', async ({ page, browser }) => {
    ...
    // Fetch OTP from CDM using CDM_OTP_URL
    console.log('Fetching OTP from CDM');
    const otp = await getOtp({ browser, type: 'login' });
    await enterOtp(page, otp);
    console.log('OTP entered');
    
    // Enter PIN from environment or default to 0000
    ...
    // PHASE 1: Personalise My Products Flow
    console.log('=== PHASE 1: Starting Personalise My Products Flow ===');
    
    // Look for the "Personalise My Products" button
    ...
    if (await personaliseButton.count()) {
      console.log('Personalise My Products button found - starting flow');
      
      // Click "Personalise My Products" button
              await personaliseButton.click({ timeout: parseInt(process.env.UNIVERSAL_CLICK_TIMEOUT || '3000', 10), force: true });
      ...
      console.log('=== PHASE 1 COMPLETED: Personalise My Products flow finished ===');
      
      // Wait a bit for the flow to complete
      await page.waitForTimeout(parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10));
      
    } else {
      console.log('Personalise My Products button not found - user may have already completed this flow');
    }
    
    // PHASE 2: Start Earning Flow
    console.log('=== PHASE 2: Starting Start Earning Flow ===');
    
    // Look for and click the "Start Earning" button
    console.log('Looking for Start Earning button');
    
    try {
      const startEarningButton = page.locator([
        'button:has(h1:has-text("Start Earning!"))',
        'button:has(h1:has-text("Start Earning"))',
        'button:has-text("Start Earning!")',
        'button:has-text("Start Earning")',
        'button:has-text("start earning")',
        '[role="button"]:has-text("Start Earning!")',
        '[role="button"]:has-text("Start Earning")',
        'div[tabindex="0"]:has(h1:has-text("Start Earning!"))',
        'div[tabindex="0"]:has(h1:has-text("Start Earning"))'
      ].join(', ')).first();
      
      await startEarningButton.waitFor({ timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) });
      console.log('Start Earning button found');
      
      // Wait until the button is clickable
      const handle = await startEarningButton.elementHandle();
      if (handle) {
        ...
        }, handle, { timeout: parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10) }).catch(() => {});
      }
      
      // Click the Start Earning button
      await startEarningButton.click({ force: true });
      console.log('Start Earning button clicked successfully');
      
      // Wait for the action to complete
      await page.waitForTimeout(parseInt(process.env.UNIVERSAL_WAIT_TIMEOUT || '3000', 10));
      
      console.log('=== PHASE 2 COMPLETED: Start Earning flow finished ===');
      
    } catch (error) {
      console.log('Start Earning button not found or not clickable:', error.message);
      throw error;
    }
    
    // Final verification
    console.log('=== ONBOARDING COMPLETED SUCCESSFULLY ===');
    console.log('Both Personalise My Products and Start Earning flows have been completed');
